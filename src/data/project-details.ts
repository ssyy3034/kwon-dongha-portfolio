...
return completePaymentProcess(orderId, paymentKey, tossResponse.method());`,
        ...
        },
        {
          ...
          problem:
            "**AI 코드 리뷰를 통해 트랜잭션 경합 시 발생하는 갱신 손실(Lost Update)과 이중 차감 취약점을 인지하게 되었습니다.**\n동일 계정에 동시 결제가 들어오면, 양쪽 트랜잭션이 차감 전 잔액을 읽고 각자 차감한 결과를 기록해 한쪽 차감이 사라지는 문제가 있었습니다. 또한 네트워크 타임아웃으로 클라이언트가 결제를 재시도할 경우, 이미 처리된 요청인지 판별하지 못해 같은 금액이 이중 차감되는 위험도 있었습니다.\nTestcontainers로 실제 DB를 띄우고 100개 스레드가 동시에 같은 계정에 결제를 보내는 테스트를 구축했고, 락 없이 실행하자 잔액이 예상보다 높게 남는 갱신 손실을 재현하며 결함을 확인했습니다.",
          approach:
            "**정합성이 금전과 직결되는 결제 도메인 특성을 고려해, 재시도 비용이 큰 낙관적 락 대신 비관적 락을 선택하고 멱등키를 병행했습니다.**\n낙관적 락은 충돌 시 애플리케이션이 재시도해야 하는데, 결제에서는 재시도 자체가 PG 이중 승인을 유발할 수 있어 부적합하다고 판단했습니다.\n- **동시성 제어**: 비관적 락으로 트랜잭션을 직렬화하되, 테이블 락이 아닌 인덱스 기반의 행 단위 락으로 점유 범위를 좁혀 서로 다른 유저의 결제는 병렬 처리되도록 설계했습니다.\n- **멱등성 보장**: 멱등키를 도입하여 DB UNIQUE 제약으로 내부 중복을 차단하고, 동일한 키를 토스페이먼츠 API 헤더에도 전달하여 PG 통신 구간까지 이중 승인을 방지했습니다.",
          result:
            "**100스레드 동시 경합에서 단 한 건의 오차 없이 잔액 정합성 100%를 통과했습니다.**\n동일 멱등키로 들어온 중복 결제 요청도 트랜잭션 진입 전에 즉시 차단됨을 확인했습니다. 현재 이 동시성 검증 테스트는 CI 파이프라인에 통합되어, 결제 로직 수정 시 회귀 오류를 자동으로 감지하고 있습니다.",
          retrospective:
...
Optional<Credit> findByUserIdWithLock(UUID userId);`,
      ...
      backend: [
        {
          ...
          problem:
            "**ML 이미지 합성(~30초) 작업이 Tomcat 스레드를 직접 점유하여, 동시 사용자 10~20명 수준에서도 전체 서비스가 마비되었습니다.**\nFlask ML 서버에 동기 호출을 보내고 응답을 기다리는 구조여서, 한 요청이 30초씩 스레드를 점유하면 일기 작성·건강 기록 등 AI와 무관한 API까지 스레드를 할당받지 못해 전부 무응답 상태에 빠졌습니다.\nk6 부하 테스트 결과 최대 처리량이 **1.16 TPS**에 불과했습니다.",
          approach:
            "**@Async로 먼저 시도한 뒤, 부하 테스트에서 한계를 확인하고 RabbitMQ로 전환했습니다.**\n`@Async`로 톰캣 스레드를 즉시 반환하도록 전환하자 처리량은 개선됐으나, 부하가 몰리자 JVM 내부 큐가 포화되어 메모리 고갈이 발생했습니다. 서버 재배포 시 메모리 큐의 미처리 작업이 유실되는 구조적 한계도 확인했습니다.\n이에 RabbitMQ를 도입하여 메시지 영속성을 확보하고, AI 합성 처리를 WAS와 물리적으로 분리했습니다. WAS는 큐에 메시지를 발행하고 즉시 응답을 반환하며, Python Worker가 메시지를 소비해 ML 처리를 수행합니다. Worker 완료 시 Webhook으로 Spring Boot에 콜백하여 결과를 저장하고, 클라이언트는 폴링으로 완료 여부를 확인하는 구조로 설계했습니다.\nWorker 장애 시에는 DLQ로 메시지를 보존하고, 메시지 재전달에 대비한 멱등성 가드를 각 단계에 적용했습니다.",
          result:
            "**WAS 처리량 1.16 → 1,949 TPS, 동기 대기 30초 → 비동기 즉시 응답(4.9ms)으로 개선했습니다.**\n500 VU 부하에서 p95 318ms, 총 175,463건을 오류 없이 처리했습니다. Flask 서버를 의도적으로 중단시킨 상태에서도 코어 API는 정상 응답하는 장애 격리를 확인했고, 미처리 메시지는 큐에 보존되어 복구 후 자동으로 재처리되었습니다.",
          retrospective:
  ...
  W->>C: 폴링으로 완료 알림`,
...

        {
          ...
          problem:
            "**주차별 정보와 오늘의 질문 기능이 매 요청마다 Gemini API를 호출하고 있어, 응답 시간 평균 487ms에 API 비용이 요청 수에 비례해 증가하는 구조였습니다.**\n성능 문제 외에 기능적 결함도 있었습니다. 주차별 정보는 주차 번호만으로 Gemini를 호출해 42주 고정 콘텐츠를 생성하고 있어, 같은 20주차라도 최근 감정이 불안한 산모와 안정적인 산모가 동일한 정보를 받았습니다. 오늘의 질문은 기획 의도상 하루 하나의 질문을 모든 산모가 공유해야 하는데, LLM 특성상 요청마다 다른 질문이 생성되는 결함이 있었습니다.",
          approach:
            "**같은 주차·같은 건강 상태의 산모에게는 동일한 응답이 돌아오므로, 상태가 바뀌지 않은 요청은 캐싱할 수 있다고 판단했습니다.**\n먼저 산모의 최근 7일 일기 감정 빈도와 최신 건강 기록을 수집해 Gemini 프롬프트에 주입하여 개인화했습니다. 그런데 개인화로 인해 상태 조합이 다양해지고 변화 시점도 불규칙하여, 고정 TTL 캐시로는 무효화 시점을 제어할 수 없었습니다.\n\n처음에는 원시 컨텍스트를 그대로 SHA-256으로 해싱했으나, 혈압이 1mmHg만 변해도 새 키가 생겨 캐시 히트율이 급락했습니다. 이에 감정(긍정/중립/부정)·혈압(정상/주의/위험)·태동(활발/보통/저조) 등 도메인 기준으로 카테고리화한 뒤 건강 상태와 주차정보를 조합해 해시 키로 사용하여, 의미 있는 상태 변화에서만 캐시가 무효화되는 구조를 설계했습니다. Caffeine 로컬 캐시를 선택해 네트워크 홉 없이 즉시 반환하도록 했습니다.\n\n오늘의 질문은 하루 1회 생성되는 공통 데이터이므로 캐싱 대상에서 분리하고, 날짜 키로 DB에 저장해 Gemini 호출을 일 1회로 고정했습니다.",
          result:
            "**주차별 정보 응답 487ms → 0.1ms(Caffeine HIT), 42개 주차별 유저 분포 시뮬레이션에서 캐시 히트율 95.80%를 달성했습니다.**\n오늘의 질문은 Gemini 호출이 일 1회로 고정되었고, 주차별 정보는 산모의 감정·건강 상태에 따라 다른 맞춤 응답을 제공하게 되어 개인화와 비용 절감을 동시에 달성했습니다.",
          retrospective:
